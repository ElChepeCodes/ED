...
/*  //Iterativo
    public static DoublyLinkedOrderedList<Divulgacion> obtieneListaRevistas
        ...
        if( material == null || material.isEmpty() ) {
            throw new EmptyCollectionException("materia esta null o vacio.");
        }
        
        // Variables y Referencias requeridas
        DoublyLinkedOrderedList<Divulgacion> listdi =
                new DoublyLinkedOrderedList();
        MaterialBibliografico auxmb = null;
        Divulgacion auxd = null;        
        Iterator<MaterialBibliografico> mbit = material.iterator();
        
        while( mbit.hasNext() ) {
            auxmb = mbit.next();
            if( auxmb instanceof Divulgacion ) {
                auxd = (Divulgacion) auxmb;
                if( (auxd.getTiraje()>total) && (auxd.getPrecio()<valprecio) ) {
                    listdi.add(auxd);
                }
            }
        }
...
*/
    ...
    public static DoublyLinkedOrderedList<Divulgacion> obtieneListaRevistas
        ...
        // Variables y Referencias requeridas
        DoublyLinkedOrderedList<Divulgacion> listdi = null;
        Iterator<MaterialBibliografico> mbit = material.iterator();
        
        listdi = obtieneListaRevistas(mbit, total, valprecio); 
            
        return listdi;
    }

    private static DoublyLinkedOrderedList<Divulgacion> obtieneListaRevistas
        (Iterator<MaterialBibliografico> mbit, int total, double valprecio){
        ...
        if( !mbit.hasNext() ) {
            // Caso Base
            listdi = new DoublyLinkedOrderedList();
        } else {
            auxmb = mbit.next();
            listdi = obtieneListaRevistas(mbit, total, valprecio);
            if( auxmb instanceof Divulgacion ) {
