...
public class Problema46a14xLSStacksMain {
    ...
    private static <T> int ctaElemPilaRv(LnkStrStack<T> pila,
        ...
        //Cuenta la cantidad de elementos y copia la pila original a la auxiliar.
        if ( flag && !pila.isEmpty() ) {
            pilaAux.push( pila.pop() );
            cant = 1 + ctaElemPilaRv(pila, pilaAux, true);
        } else {
            //Regresa valores a la pila original.
            if ( !pilaAux.isEmpty() ) {
                pila.push(pilaAux.pop());
                cant = ctaElemPilaRv(pila, pilaAux, false);
            } else {
                cant = 0;
            }
        }
