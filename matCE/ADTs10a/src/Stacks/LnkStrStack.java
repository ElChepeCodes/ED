...
public class LnkStrStack<T> implements StackADT<T> {
    ...
    public static <T> int ctaElemRv(LnkStrStack<T> pila){
        if (pila==null)
            throw new ParamErrException("<pila> es null");
        LnkStrStack <T> aux= new LnkStrStack();
        int cont=0;
        cont =cuentaElemRv(pila, aux, cont, true);
        
                
        return cont;
    }
    
    public static <T> int cuentaElemRv(LnkStrStack<T> pila, LnkStrStack<T> aux, int cont, boolean state){        
        if (!pila.isEmpty() && state){
            cont++;
            aux.push(pila.pop());
            cont =cuentaElemRv(pila, aux, cont, state);
        }
        else{    
            if (!aux.isEmpty())
            pila.push(aux.pop());
            cuentaElemRv(pila, aux, cont, false);
        }
