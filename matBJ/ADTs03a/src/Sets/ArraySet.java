...
*/
  ...
  public SetADT<T> intersection(SetADT<T> oSet) {
    ...
    public SetADT<T> intersectionRv(SetADT<T> oSet) {
        if(oSet==null)
            throw new ParamErrException("<nulo o fuera de rango>");
        SetADT<T> result= null;
        Iterator<T> iter1 = this.iterator(), iter2=oSet.iterator();
    ...
    private SetADT<T> intersectionRv(Iterator<T> oSetIter, SetADT<T> interSet) {
        ...
        if (oSetIter.hasNext()){
            temp=oSetIter.next();
            if (this.containsRv(temp))
                interSet.add(temp);
            interSet.addAll(intersectionRv(oSetIter,interSet));
        }//if
        return interSet;
    }//method
  ...
  public SetADT<T> difference(SetADT<T> oSet) {
    ...
    private SetADT<T> differenceRv(Iterator<T> iter, SetADT<T> oSet,
        ...
        if (iter.hasNext()){
            temp=iter.next();
            if (!((ArraySet)oSet).containsRv(temp))
                diffSet.add(temp);
            diffSet.addAll(differenceRv(iter,oSet, diffSet));
        }//if
        return diffSet;
